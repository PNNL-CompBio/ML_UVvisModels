...
# RJ get bond pairs
        ...
        for atom1 in range(self.n_atoms):
            for atom2 in range(atom1 + 1, self.n_atoms):
                bond = mol.GetBondBetweenAtoms(atom1, atom2) # check if this and few lines below can be merged
                if bond is None:
                    continue
                else:
                    all_bonds_pairs.extend([[atom1, atom2]])
        dij = dijkstra_distance(all_bonds_pairs)
        print(dij)
#
        ...
        for a1 in range(self.n_atoms):
            for a2 in range(a1 + 1, self.n_atoms):
                ...
                G_dist = []
                if bond is None: # check the effect of this, remove it to check 
                    r_ij = rdkit.Chem.rdMolTransforms.GetBondLength(conf, a1, a2)
                    for k in range(len(gaus_dist)):
                        gaussian = np.exp(-(r_ij - gaus_dist[k])**2/0.5**2)
                        G_dist.append(gaussian)
                    print('non-bonding G_dist',len(G_dist))
                else:
                    r_ij  = rdkit.Chem.rdMolTransforms.GetBondLength(conf, a1, a2)
                    for k in range(len(gaus_dist)):
                        gaussian = np.exp(-(r_ij - gaus_dist[k])**2/0.5**2)
                        G_dist.append(gaussian)
                    print('bonding G_dist',len(G_dist))
...
# rj samering pairs
                ...
                for ring in ring_info:
                    ispart.append(set(bond_pair).issubset(ring))
                samering = 0
                if any(ispart)==True:
                    samering = 1
# rj
                dijk = []
                G_dist.append(samering)
                G_dist.append(dij[a1, a2])
                dijk.append(samering)
                dijk.append(dij[a1, a2])
